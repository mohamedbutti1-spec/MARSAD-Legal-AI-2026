...
// PORT is only needed for the dev/preview server — not for a static production build.
const rawPort = process.env.PORT;
const isProductionBuild = process.env.NODE_ENV === 'production' && !process.env.PORT;
...
const port = rawPort ? Number(rawPort) : 23979; // fallback only used in prod build (ignored)
